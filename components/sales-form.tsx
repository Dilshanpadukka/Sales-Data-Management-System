...
export function SalesForm() {
  const { addSalesRecord, exportToCSV, settings } = useAppContext()
  const [calculatedAmount, setCalculatedAmount] = useState<number | null>(null)
  const [defaultUnitPrice, setDefaultUnitPrice] = useState<number>(0)

  // Dynamically create form schema based on visible fields
  const createFormSchema = () => {
    ...
    settings.customFields.forEach((field) => {
      if (field.type === "text") {
        ...
        schemaObj[field.id] = field.required ? z.coerce.number() : z.coerce.number().optional()
  ...
  useEffect(() => {
    if (selectedItem) {
      ...
      if (itemData) {
        ...
        setDefaultUnitPrice(itemData.defaultPrice)
      }
    }
  ...
  function onSubmit(values: any) {
    ...
    // Create a complete record with all fields, even if some are not visible
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {settings.fieldVisibility.saleAmount && (
              <div className="flex flex-col space-y-2">
                ...
                <div className="h-10 px-3 py-2 rounded-md border border-input bg-gray-100 text-sm">
                  {calculatedAmount !== null ? `$${calculatedAmount.toFixed(2)}` : "$0.00"}
                </div>
          ...
          <div className="flex justify-between">
            ...
            <Button type="button" variant="outline" onClick={exportToCSV} className="w-32">
              Export CSV
            </Button>
          </div>
        </form>
